async function run() {
    ...
    var address = 'TLJQwXkmYTU5ahRLpmXK63oFmzEadvVyRM'; // Mainnet
    // var address = 'TB2wzW2owPsahfwu4YwhxJXU2Pc9CrhfLa'; // Shasta Testnet
    ...
    function init() {
        ...
        $('#stake').click(async function (event) {
            event.preventDefault();
            var amount = parseInt($('#stakeAmount').val());
            let value = amount.toString() + "000000";
            console.log(value)
            if (amount > 0) {try {await FLUX.stake(value).send();} catch (err) {console.error(err)}}
        });
        $('#unstake').click(async function (event) {
            ...
            let value = amount.toString() + "000000";
            console.log(value)
            if (amount > 0) {try {await FLUX.unstake(value).send();} catch (err) { console.error(err)}}
        });
    ...
    function update() {
        ...
        FLUX.allInfoFor(account).call(function (error, info) {
          if (!error) {
              ...
              $('#totalStaked').text(formatNumber(parseFloat(info.totalTokensStaked / 1e6), 5));
              $('#myTokens').text(formatNumber(parseFloat(info.userBalance / 1e6), 5));
              $('#myStaked').text(formatNumber(parseFloat(info.userStaked / 1e6), 5));
